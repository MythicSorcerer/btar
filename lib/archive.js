...
export async function extractArchive(archivePath, outputDir = '.') {
  ...
  if (isGzipped) {
    ...
    throw new Error('.xz format not supported - requires xz package. Use .tar.gz instead.');
  } else {
  ...
  while (offset + 512 <= tarData.length) {
    ...
    if (fileName && fileSize > 0) {
      ...
      if (outputDir === null) {
        ...
        // Clean the filename - remove leading slashes and resolve within output dir
        const cleanName = fileName.replace(/^[\/\\]+/, '').replace(/\.\.[\/\\]/g, '');

        // Build and validate the output path
        let outPath = join(resolvedOutputDir, cleanName);
        const resolvedOutPath = resolve(outPath);

        // Ensure the resolved path is within the output directory (path traversal protection)
        if (!resolvedOutPath.startsWith(resolvedOutputDir + '/') && resolvedOutPath !== resolvedOutputDir) {
          console.warn(`Skipping potentially dangerous file: ${fileName}`);
          continue;
        }
