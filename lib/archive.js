...
export async function createArchive(files, outputPath) {
  ...
  // Check if output should be compressed (.tar.gz or .tar.xz)
  ...
  } else if (outputPath.endsWith('.xz')) {
    // For .xz we need to write uncompressed tar as .tar.xz
    const { createXZ } = await import('node:zlib');
    const xz = createXZ();
    const out = createWriteStream(outputPath);
    const input = Readable.from(tarBuffer);
    await pipeline(input, xz, out);
  } else {
    // Plain tar
...
export async function extractArchive(archivePath, outputDir = '.') {
  ...
  // Extract files from tar data
  const extracted = extractTarContents(tarData, outputDir);
  return `Extracted ${extracted.length} file(s) to ${outputDir}`;
}
